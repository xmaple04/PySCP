...
class Vehicle:
    ...
    def objective(self, x):
        ...
        # dxInt = np.matmul(self.lgr.PIM, 1)
        dxDiff = np.matmul(self.lgr.PDM, state) - tf / 2 * self.fAll(state, control)
        linecost = np.sqrt(np.sum(dxDiff ** 2, axis=1))
        cost = np.matmul(self.lgr.weights4Control, linecost)
        return cost

    def fAll(self, state, control):
        f = np.zeros((self.ncp, self.xdim))
        for i in range(self.ncp):
            x = state[i + 1]
            u = control[i]
            f[i] = self.RealDynamics(x, u)
        return f

    def fABAll(self, state, control):
        f = np.zeros((self.ncp, self.xdim))
        ...
        for i in range(self.ncp):
            ...
            u = control[i]
            f[i] = self.RealDynamics(x, u)
            A[i], B[i] = self.ApproxDynamics(x, u)
        return f, A, B

    def RealDynamics(self, x, u):
        return np.array([x[1], u[0]])

    def ApproxDynamics(self, x, u):
        A = np.array([[0, 1], [0, 0]])
        B = np.array([[0], [1]])
        return A, B
    ...
    def gradient(self, x):
        ...
        f, A, B = self.fABAll(state, control)
        # xgrad
...
if __name__ == '__main__':
    ...
    tflb = 1.
    tfub = 2.
    umaxList = np.linspace(1.9, 2.5, 50)
    cost = np.zeros_like(umaxList)
    for i in range(len(umaxList)):
        uub = [umaxList[i]]
        ...
        cost[i] = vehicle.objective(x)

    plt.figure()
    plt.plot(umaxList, cost, marker='o')
    plt.yscale('log')
    plt.show()

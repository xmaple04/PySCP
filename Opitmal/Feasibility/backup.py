...
class Vehicle:
    ...
    def getInitVector(self, xlb, xub, xflb, xfub, ulb, uub, tflb, tfub):
        ...
        x0 = np.concatenate([state, np.ones_like(control) * 2, [2]])
        # self.plot(x0)
        # self.plot(lb)
        # self.plot(ub)
        # x = self.objective(x0)
...
if __name__ == '__main__':
    ...
    #               # [-4., 3., 5.],
    #               [1., 2., 5.]])
    # k = vehicle.vector2Dgrad(x)
    # print(k)

    xlb = np.array([0., 0.])
    xub = np.array([6., 5.])
    tol = 0.01
    xflb = np.array([4 - tol, 4 - tol])
    xfub = np.array([4 + tol, 4 + tol])
    ulb = 0.
    uub = 2.
    tflb = 1. - tol
    tfub = 2. + tol
    ...
    nlp.add_option('acceptable_obj_change_tol', 1e-11)
